...
--GO
 ...
 ALTER PROCEDURE sp_AddScheduleForBaseStacionar
 --@group_name			NVARCHAR(50),
 ----@discipline_name		NVARCHAR(256),
 --@start_date			DATE,
 --@group_time			TIME,
 ...
 --BEGIN
	--	DECLARE @group					AS INT		= (SELECT group_id FROM Groups WHERE group_name = @group_name);
	--	DECLARE @teacher_1				AS INT		= (SELECT teacher_id FROM Teachers WHERE last_name = @teacher_1_last_name);
	--	DECLARE @teacher_2				AS INT		= (SELECT teacher_id FROM Teachers WHERE last_name = @teacher_2_last_name);
	--	DECLARE @date					AS DATE		= @start_date;
	--	DECLARE @discipline_1			AS SMALLINT = (SELECT discipline_id FROM Disciplines WHERE discipline_name LIKE '%Hardware%')
	--	DECLARE @discipline_2			AS SMALLINT = (SELECT discipline_id FROM Disciplines WHERE discipline_name LIKE '%Процедурное программирование %C++%')
	--	DECLARE @number_of_lessons_1	AS SMALLINT = (SELECT number_of_lessons FROM Disciplines WHERE discipline_name LIKE '%Hardware%')
	--	DECLARE @number_of_lessons_2	AS SMALLINT = (SELECT number_of_lessons FROM Disciplines WHERE discipline_name LIKE '%Процедурное программирование %C++%')
